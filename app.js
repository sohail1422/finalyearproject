...
// Server-side certificate generation route
app.get('/certificate', (req, res) => {
  if (!req.session.user) return res.redirect('/login');
  const userRecord = users.find(u => u.username === req.session.user) || { username: req.session.user, name: req.session.user };
  const userAchievements = achievements.filter(a => a.user === req.session.user).slice().sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0));
  const tierInfo = badgeTierInfo(userAchievements.length);

  res.setHeader('Content-Disposition', `attachment; filename="enrichment-certificate-${userRecord.username}.pdf"`);
  ...
  // Header
  doc.fontSize(20).text('Enrichment Passport Certificate', { align: 'left' });
  doc.moveDown(0.5);

  // Badge image (if available)
  try {
    const badgePath = tierInfo.badge.replace(/^\//, '');
    const fullBadgePath = path.join(__dirname, 'public', badgePath);
    if (fs.existsSync(fullBadgePath)) {
      doc.image(fullBadgePath, 450, 40, { fit: [80, 80], align: 'right' });
    }
  } catch (e) {
    // ignore image errors
  }
  ...
  doc.text(`Name: ${userRecord.name || userRecord.username}`);
  doc.text(`Username: ${userRecord.username}`);
  doc.text(`Tier: ${tierInfo.tier}`);
  ...
  if (userAchievements.length === 0) {
    doc.text('No achievements recorded yet.');
  } else {
    userAchievements.forEach((a, idx) => {
      const dateLabel = a.createdAt ? new Date(a.createdAt).toLocaleDateString() : 'Unknown';
      doc.fontSize(11).text(`${idx + 1}. ${a.title} — ${dateLabel}`);
      if (a.notes) doc.fontSize(10).text(`   ${a.notes}`);
    });
  }

  doc.moveDown(1);
  doc.fontSize(10).text('Generated by Enrichment Passport', { align: 'left' });

  doc.end();
});

// Certificate preview page (HTML preview and download link)
app.get('/certificate/preview', (req, res) => {
  if (!req.session.user) return res.redirect('/login');
  const user = users.find(u => u.username === req.session.user) || { username: req.session.user, name: req.session.user };
  const userAchievements = achievements.filter(a => a.user === req.session.user).slice().sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0)).map(a => ({ ...a, dateLabel: a.createdAt ? new Date(a.createdAt).toLocaleDateString() : 'Unknown' }));
  const tierInfo = badgeTierInfo(userAchievements.length);
